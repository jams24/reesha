    ...
    <div className="container-x py-10 sm:py-14">
      ...
      {items.length === 0 ? (
        <div className="py-24 text-center">
          <p className="text-neutral-500 mb-6">You haven't saved anything yet.</p>
          ...
          {items.map((item) => (
            <div key={item.slug} className="group">
              <Link to={`/product/${item.slug}`} className="block">
                <div className="relative aspect-[3/4] bg-neutral-100 overflow-hidden">
                  {item.image ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-xs uppercase tracking-widest2 text-neutral-400">No image</div>
                  )}
                </div>
                <div className="mt-3">
              ...
              <button
                ...
                className="mt-2 text-[11px] uppercase tracking-widest2 text-neutral-500 hover:text-ink underline underline-offset-4"
              >
                Remove
              </button>
            </div>
