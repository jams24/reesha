...
export default function HomePage() {
    ...
    (async () => {
      try {
        ...
        if (items.length < 4) {
          ...
          const existing = new Set(items.map((i) => i._id));
          items = [...items, ...(more.items || []).filter((i) => !existing.has(i._id))].slice(0, 8);
        }
  ...
  return (
    <div>
          ...
          {CATEGORY_TILES.map((c) => (
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 text-paper">
                ...
                <p className="text-[10px] uppercase tracking-widest2 opacity-80">Shop now →</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="container-x py-16 border-t border-neutral-200">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
      ...
      <section className="bg-ink text-paper">
        <div className="container-x py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-[11px] uppercase tracking-widest2 text-neutral-400">About Reesha</p>
          ...
          <div className="space-y-6 text-neutral-300 leading-relaxed">
            ...
            <Link to="/about" className="inline-flex items-center gap-2 text-sm uppercase tracking-widest2 border-b border-paper pb-1 hover:text-neutral-400 hover:border-neutral-400 transition">
              Our story
            </Link>
          </div>
        </div>
